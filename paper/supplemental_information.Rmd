...
Table \ref{tab:dataset-table-supp} has additional information on how many trials each dataset contributed and per-dataset exclusion rates for trials.
...
dataset_characteristics |> left_join(clean_names) |> left_join(dataset_pcts) |>
  ...
  mutate(`Pct Acc Trials`=str_c(sprintf(fmt = "%01.1f", pct_trials_acc ), "%"),
         ...
         ` `=row_number() |> as.character())|> dplyr::select(` `, `Dataset Name`, `Pct Acc Trials`, `Pct RT Trials`, `Total Acc Trials`=total_trial_acc,  `Total RT Trials`=total_trial_rt, `Include rate (LW)`,
`Include rate (SW)`,`Include rate (RT)`,`N subjects`, `N admins`) |>
  
  knitr::kable(caption='Characteristics of included datasets from Peekbank.', align="rlrrrrrrrr", digits=2, booktabs=T, linesep="") |>
  kableExtra::kable_styling(latex_options="scale_down")|> row_spec(27, hline_after=T)
```
