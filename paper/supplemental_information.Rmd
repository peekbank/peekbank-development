...
dataset_characteristics |> left_join(clean_names) |> left_join(dataset_pcts) |>
  ...
  mutate(`Pct Acc Trials`=str_c(sprintf(fmt = "%01.1f", pct_trials_acc ), "%"),
         `Pct RT Trials`=str_c(sprintf(fmt = "%01.1f", pct_trials_rt ), "%"),
          `Include rate (LW)`=str_c(sprintf(fmt = "%01.1f", valid_long_window_rate ), "%"),
          `Include rate (SW)`=str_c(sprintf(fmt = "%01.1f", valid_short_window_rate ), "%"),
          `Include rate (RT)`=str_c(sprintf(fmt = "%01.1f", valid_rt_rate ), "%"),


         ` `=row_number() |> as.character())|> select(` `, `Dataset Name`, `Pct Acc Trials`, `Pct RT Trials`, `Total Acc Trials`=total_trial_acc,  `Total RT Trials`=total_trial_rt, `Include rate (LW)`,
`Include rate (SW)`,`Include rate (RT)`,
                                                      `N subjects`, `N admins`, everything()) |> View()
  
  
  # kable(caption='Characteristics of included datasets from Peekbank. "Admins" denotes separate experimental sessions. \"CDIs\" refers to whether the dataset contains parent report vocabulary data from the MacArthur-Bates Communicative Development Inventory. \\label{tab:datasets}', align="rlrrrrrrrrr", digits=2, booktabs=T, linesep="") |> 
