...
affiliation:
  ...
  - id: "3"
    institution: Department of Psychology, University of Wisconsin
  - id: "4"
    institution: Department of Linguistics, University of California
  - id: "5"
    institution: Department of Communication, Stanford University
  - id: "6"
    institution: Department of Psychology, Ludwig Maximilian University
  - id: "7"
...
author_contributions: |
  MCF contributed to conceptualization, data curation, formal analysis, funding acquisition, investigation, methodology, project administration, software, supervision, visualization, writing – original draft, and writing – review & editing. VAM contributed to conceptualization, data curation, investigation, methodology, and writing – review & editing. CAB contributed to conceptualization, data curation, investigation, methodology, software, and writing – review & editing. VB contributed to data curation, software, visualization, and writing – review & editing. MB contributed to software. GK contributed to data curation, investigation, software, validation, and writing – review & editing. JM contributed to data curation, software, and validation. SM contributed to conceptualization, data curation, methodology, software, and writing – review & editing. BP contributed to data curation and software. NR contributed to formal analysis, visualization, and writing – review & editing. RZS contributed to data curation, validation, and writing – review & editing. AS contributed to data curation and software. AWMT contributed to data curation, software, and writing – review & editing. MZ contributed to conceptualization, data curation, formal analysis, funding acquisition, investigation, methodology, project administration, software, supervision, validation, visualization, and writing – review & editing.
...
acknowledgements: |
  This work was supported by a grant from the Jacobs Foundation Creating Impact Science Program (CRISP). 
...
d_sub_firstadmin <- d_sub |>
  ...
  filter(admin_num == 1) |>
  ungroup() |>
  dplyr::select(-admin_num, -time_since_t0, -delta_t)
...
dataset_characteristics <- d_sub |>
  ...
  mutate(`Avg RT Trials`=ifelse(is.na(`Avg RT Trials`), 0, `Avg RT Trials`))

dataset_pcts <- d_sub |>
  group_by(dataset_name) |>
  summarise(total_trial=sum(n_trials[!is.na(long_window_accuracy)])) |>
  ungroup() |> 
  mutate(pct_trials=round(total_trial/sum(total_trial)*100, 1)) |> 
  dplyr::select(-total_trial)

clean_names <- tribble(
  ~dataset_name, ~`Dataset Name`,
  "reflook_v4", "Yurovsky et al. (2017)", 
  "yurovsky_2017", "Yurovsky & Frank (2017)", 
  "weaver_zettersten_2024", "Weaver et al. (2024)",
  "fernald_marchman_2012", "Fernald & Marchman (2012)",
  "frank_tablet_2016", "Frank et al. (2016)", 
  "fmw_2013", "Fernald et al. (2013)", 
  "adams_marchman_2018", "Adams et al. (2018)" ,
  "potter_canine", "Potter & Lew Williams (2024)",
  "fernald_totlot", "Fernald et al. (2006)",
"pomper_prime", "Pomper & Saffran (2015)",
"pomper_saffran_2016", "Pomper & Saffran (2016)", 
"swingley_aslin_2002", "Swingley & Aslin (2002)",
"pomper_salientme", "Pomper & Saffran (2019)", 
"perry_cowpig", "Perry & Saffan (2017)", 
"ronfard_2021", "Ronfard et al. (2022)",
"bacon_gendercues", "Bacon & Saffran (2022)",
"garrison_bergelson_2020", "Garrison et al. (2020)",
"pomper_yumme", "Pomper & Saffran (2018)", 
"mahr_coartic", "Mahr et al. (2015)",
"reflook_socword", "Yurovsky et al. (2013)",
"yoon_simpimp_2015","Yoon et al. (2015)",
"borovsky_2019", "Borovsky & Peters (2019)",
"pomper_dimy", "Pomper & Saffran (2017)",
"weaver_saffran_2026", "Weaver & Saffran (2026)",
"bergelson_swingley_2012", "Bergelson & Swingley (2012)",
"moore_bergelson_2022_verb", "Moore & Bergelson (2022)",
)
dataset_characteristics |> left_join(clean_names) |> left_join(dataset_pcts) |> arrange(desc(pct_trials)) |> 
  mutate(`Pct Trials`=str_c(sprintf(fmt = "%01.1f", pct_trials ), "%"),
         ` `=row_number() |> as.character())|> dplyr::select(` `, `Dataset Name`, `Pct Trials`, everything()) |> dplyr::select(-dataset_name, -pct_trials) |> 
  bind_rows(tibble(` `="", `Dataset Name` = "Total", 
                   `Pct Trials`="100%",
                       ...
                       `longitudinal` = as.character(sum(dataset_characteristics$longitudinal != "")))) |> kable(caption='Characteristics of included datasets from Peekbank. "Admins" denotes separate experimental sessions. \"CDIs\" refers to whether the dataset contains parent report vocabulary data from the MacArthur-Bates Communicative Development Inventory. \\label{tab:datasets}', align="rlrrrrrrrrr", digits=2, booktabs=T, linesep="") |> 
  kable_styling(latex_options="scale_down") |> row_spec(26, hline_after=T)
```
...
mycolors = c("#1B9E77", "#D95F02", "#E7298A", "#66A61E", "#1B9E77", "#D95F02", "#E7298A", "#66A61E", "#1B9E77", "#D95F02", "#E7298A", "#66A61E","#1B9E77", "#D95F02", "#E7298A", "#66A61E","#1B9E77", "#D95F02")
...
parallel_ana <- capture.output(fa.parallel(dplyr::select(d_sub_mat, -dataset_name, -age, -log_age), fa = "fa", 
...
Figure \ref{fig:age-regression} shows a regression model fit to this confirmatory factor analysis, with log age predicting each latent variable. This regression model allows interpretation of the covariances between latent factors as partial correlations (controlling for age). The non-age related variance of all three latent factors was significantly related to that of the other factors. Speed and accuracy showed strong negative covariance ($\beta=$ `r round(fit3_age_summary$pe[21,"est"],3)`, SE = `r round(fit3_age_summary$pe[21,"se"],3)`, $p$ `r fit3_age_summary$pe[21,"pvalue"] |> apa_p(add_equals=T)`), as expected since they are derived from the same data. Importantly, there was also weaker but significant covariation between RT and vocabulary ($\beta=$ `r round(fit3_age_summary$pe[20,"est"],3)`, SE = `r round(fit3_age_summary$pe[20,"se"],3)`, $p$ `r fit3_age_summary$pe[20,"pvalue"] |> apa_p(add_equals=T)`) and accuracy and vocabulary ($\beta=$ `r round(fit3_age_summary$pe[19,"est"],3)`, SE = `r round(fit3_age_summary$pe[19,"se"],3)`, $p$ `r fit3_age_summary$pe[19,"pvalue"] |> apa_p(add_equals=T)`). This model supports the idea that variation in speed and accuracy of word recognition is related to individual differences in parent-reported vocabulary beyond the effects of age. Further, the broader set of analyses supports a factor structure in which speed and accuracy (and their associated variabilities) are related but distinct aspects of word recognition, rather than being measures of one single construct. These analyses treat all data as between person, however, rather than modeling change in these factors within individuals. 
...
```{r}
d_growth <- d_sub |>
  #filter(!is.na(prod), subject_id %in% longitudinal$subject_id) |> #if we want to restrict to only those children with multiple LWL sessions
  filter(!is.na(prod), !is.na(log_rt)) |>
  group_by(subject_id) |>
  arrange(age) |>
...
```{r growth, fig.cap="Growth curves from a logistic growth model showing predicted productive vocabulary growth for children with initial reaction times one SD faster than the mean (blue), at the mean (red), and one SD slower than the mean (green). Individual longitudinal trajectories are shown in light gray. Solid lines show global model estimates and colored regions indicate 95% credible intervals.", fig.env="figure", fig.height = 4, include=TRUE, warning=FALSE,message=FALSE}
...
ggplot(new_data, aes(x = age_c, y = fit, col = as.factor(log_rt_0_c))) +
  ...
  geom_ribbon(aes(ymin = lower, ymax = upper, fill = as.factor(log_rt_0_c), group = as.factor(log_rt_0_c)), 
              alpha = 0.3) +
  labs(x = "Centered Age (in months)", y = "Predicted productive vocabulary\n(fixed effects only)") +
  theme_minimal() + 
  scale_color_solarized(name = "log RT\nat t0 (SD)") + 
...
d_sub_s <- d_sub |>
  ungroup() |>
  dplyr::select(dataset_name, subject_id, administration_id, age, 
...
We retrieved all data from Peekbank release 2026.1 using the `peekbankr` package.  All code and data necessary to reproduce this manuscript are available at [https://github.com/peekbank/peekbank-development](). 
