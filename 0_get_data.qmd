...
In the interest of smaller RAM footprints, we pre-filter for age (12-60 mo), language (english), and vanilla trials. 

```{r}

all_aoi_timepoints <- readRDS(here("cached_intermediates", "raw_aois.rds"))
aoi_data_joined <- all_aoi_timepoints |> 
  inner_join(all_administrations |> select(dataset_name, dataset_id, subject_id, administration_id, age) |> filter(age>=12, age<=60)) |>
