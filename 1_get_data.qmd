...
```{r}
ggplot(d_sub, aes(x = n_trials, fill = n_trials < 4)) + 
  geom_histogram(binwidth = 1) + 
  facet_wrap(~dataset_name, scales = "free_y")
```
```{r}
ggplot(d_sub, aes(x = n_trials_rt, fill = n_trials_rt < 2)) + 
  geom_histogram(binwidth = 1) + 
  facet_wrap(~dataset_name, scales = "free_y")
```
...
```{r}
d_sub <- d_sub |>
  mutate(across(contains("log_rt"), ~ifelse(n_trials_rt < RT_MIN_TRIAL_CUTOFF, NA, .x)),
...
print(paste(length(unique(d_sub$subject_id)), "children,",
            sum(!is.na(d_sub$prod)), "with CDI data"))
