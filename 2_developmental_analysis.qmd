...
d_var <- d_trial |>
  ...
  summarise (`log(RT)` = sd(log_rt, na.rm=TRUE), 
             `Accuracy` = sd(long_window_accuracy, na.rm=TRUE)) |>
  pivot_longer(c("log(RT)","Accuracy"), 
...
d_sub |>
  ...
  summarise(`N subjects` = length(unique(subject_id)),
            ...
            `Min Age` = min(age), 
            `Max Age` = max(age), 
            `Avg Trials` = mean(n_trials),
            `Avg RT Trials` = mean(n_trials_rt),
...
```{r}
a <- ggplot(d_sub_long |> filter(measure == "long_window_accuracy"), 
       aes(x = age, y = accuracy)) + 
  geom_point(alpha = .05) + 
  geom_smooth() +
  geom_smooth(method = "lm", col = "green") +
  geom_smooth(data = d_sub_agevar_long |> filter(measure == "long_window_accuracy"),
              aes(group = dataset_name, col = dataset_name), geom="line",
              alpha = .1, method = "lm", size = .5, lty = 2) + 
  geom_hline(yintercept = .5, lty = 2) + 
  scale_x_log10() +
